...
def setCanvasSize(w=_DEFAULT_CANVAS_SIZE, h=_DEFAULT_CANVAS_SIZE):
    ...
    _canvas.bind("<ButtonRelease>", _buttonReleaseCallback)
...
# Caches the last MAX_IMAGES images loaded.
...
# keeps up to MAX_IMAGES photo images alive at any given time. 
...
def _getPhotoImageReference(w, h):
    ...
    # We're only going to cache IMAGES of these at any given time
    if len(_imageRefs) >= __MAX_IMAGES: _imageRefs.pop(0)
...
def _buttonReleaseCallback(event):
    """
    Check if a left mouse button released event has occurred
    """
    global _clicks
    ...
    _clicks = [(event.x, event.y, "left-up", time.time())] + _clicks
...
def _leftButtonCallback(event):
    ...
    _clicks = [(event.x, event.y, "left", time.time())] + _clicks    
...
def _rightButtonCallback(event):
    ...
    _clicks = [(event.x, event.y, "right", time.time())] + _clicks
...
def mouseReleased():
    """
    Return True if the most recent mouse event was the release
    of a left mouse click
    """
    global _mousePos
    if _mousePos:
        return _mousePos[2] == "left-up"
    raise Exception(
        "Can't determine mouse button if click hasn't happened") 

def mouseEventTimeStamp():
    """
    Return the time at which the mouse was most recently 
    clicked or released. If a click
    hasn't happened yet, raise an exception, since mouseEventTimeStamp()
    shouldn't be called until mousePressed() returns True.
    """    
    global _mousePos
    if _mousePos:
        return _mousePos[3] 
